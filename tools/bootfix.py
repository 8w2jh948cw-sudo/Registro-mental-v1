...
hidden_css = '#rmBootActions[hidden],#rmBootDiagnostics[hidden]{display:none!important}'

# Oficial: força um boot novo no Safari e corrige os controles ocultos.
p = Path('index.html')
s = p.read_text(encoding='utf-8')
if hidden_css not in s:
    s = replace_once(
        s,
...
s = re.sub(
    r'<script src="\./boot\.js\?v=[^"]+" defer></script>',
    '<script src="./boot.js?v=1.1.10-bootfix2" defer></script>',
    s,
    count=1
)
p.write_text(s, encoding='utf-8')

# Beta: mesma correção visual/diagnóstico e cache busting do boot.
p = Path('beta/index.html')
s = p.read_text(encoding='utf-8')
if hidden_css not in s:
    s = replace_once(
        s,
...
s = re.sub(
    r'<script src="\./boot\.js\?v=[^"]+" defer></script>',
    '<script src="./boot.js?v=1.2.0-beta.14-bootfix2" defer></script>',
    s,
    count=1
)
p.write_text(s, encoding='utf-8')

# Oficial: verificação da interface baseada no DOM real, não na exposição de funções globais.
p = Path('boot.js')
s = p.read_text(encoding='utf-8')
s = s.replace("const RELEASE = '1.1.8';", "const RELEASE = '1.1.10';", 1)
old_func = """  function appLooksFunctional() {
...
"""
new_func = """  function appLooksFunctional() {
    const shell = document.querySelector('.app-shell');
...
pattern = re.compile(r"  copyButton\?\.addEventListener\('click', async \(\) => \{.*?\n  \}\);", re.S)
replacement = """  copyButton?.addEventListener('click', async () => {
...
if count != 1:
    raise SystemExit('Handler de cópia oficial não encontrado')
p.write_text(s, encoding='utf-8')

# Beta: verificação por DOM e fallback do diagnóstico sempre visível.
p = Path('beta/boot.js')
s = p.read_text(encoding='utf-8')
old_func = """  function appLooksFunctional() {
...
s, count = pattern.subn(replacement, s, count=1)
if count != 1:
    raise SystemExit('Handler de cópia beta não encontrado')
p.write_text(s, encoding='utf-8')

# Versão visível da Oficial.
p = Path('official-appearance.js')
s = p.read_text(encoding='utf-8')
s = s.replace("const RELEASE = '1.1.9';", "const RELEASE = '1.1.10';", 1)
p.write_text(s, encoding='utf-8')

print('Boot fix aplicado com sucesso.')
