...
(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .rm-beta-mood-selector .rm-beta-mood-zero::after { content: none !important; }
      ...
      html[data-visual-mode="ultra"] .rm-beta-mood-selector .rm-beta-mood-zero.selected {
        box-shadow: inset 0 0 0 1px rgba(255,255,255,.20), 0 0 8px rgba(118,87,255,.72), 0 0 22px rgba(118,87,255,.48), 0 0 38px rgba(118,87,255,.20) !important;
      }
      .rm-beta-mood-track {
      ...
      .rm-beta-mood-fill {
        ...
        pointer-events: none;
        opacity: 0;
        background: linear-gradient(90deg, color-mix(in srgb, var(--rm-mood-color) 88%, white 12%), var(--rm-mood-color));
      ...
      html[data-visual-mode="ultra"] .rm-beta-mood-track {
        ...
        box-shadow: inset 0 0 0 .5px rgba(255,255,255,.10), 0 0 9px rgba(255,255,255,.10);
      }
      html[data-visual-mode="ultra"] .rm-beta-mood-track.has-value .rm-beta-mood-fill {
        box-shadow: inset 0 0 0 .5px rgba(255,255,255,.18), 0 0 9px var(--rm-mood-glow), 0 0 20px color-mix(in srgb, var(--rm-mood-glow) 72%, transparent) !important;
      }
      ...
      .rm-beta-mood-selector .rm-beta-mood-zero:active { transform: scale(.92) !important; }
      ...
      @media (max-width: 380px) {
        ...
        .rm-beta-mood-track .mood-score {
          width: 39px !important; height: 39px !important; min-width: 39px !important; min-height: 39px !important; font-size: 13px !important;
        }
      ...
      .rm-beta-card-moodbar.is-zero .rm-beta-card-mood-fill {
        inset: 2px; width: auto; min-width: 0; padding: 0; justify-content: center; background: #7657FF; border-color: #A994FF;
      }
  ...
  function enlargeCloseIcon() {
    document.querySelectorAll('.sheet-close').forEach(button => {
      ...
      button.innerHTML = `<svg class="rm-beta-x-icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false"><path d="M4.5 4.5L27.5 27.5M27.5 4.5L4.5 27.5" fill="none" stroke="currentColor" stroke-width="2.8" stroke-linecap="round" /></svg>`;
    });
  }
  ...
  function removeRedundantSheetCancels() {
    ...
    form.querySelectorAll(':scope > .form-actions').forEach(actions => {
      ...
      const cancel = directButtons.find(button => button.hasAttribute('data-cancel') || /^cancelar$/i.test(button.textContent.trim()));
      if (!cancel) return;
      cancel.remove();
  ...
  function refineMoodSelector() {
    ...
    observer.observe(scale, {subtree:true,attributes:true,attributeFilter:['class']});
  ...
  function observeSheetForm() {
    ...
    new MutationObserver(() => { queueMicrotask(apply); }).observe(form, { childList: true });
  }
