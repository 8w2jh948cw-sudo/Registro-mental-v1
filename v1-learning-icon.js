/* Registro Mental V1 — ícone da aba Aprendizado */
(() => {
  const ICON = `<svg class="svg-icon rm-learning-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21.7734 24.9609" width="24" height="24" aria-hidden="true" focusable="false">
    <g>
  ...
  function applyLearningIcon() {
    const tab = document.querySelector('[data-tab="learning"]');
    if (!tab) return;
    const holder = tab.querySelector('[data-icon]') || tab.querySelector('span');
    if (!holder) return;
    if (holder.dataset.rmLearningIcon === '1') return;
    holder.dataset.rmLearningIcon = '1';
    holder.removeAttribute('data-icon');
    holder.innerHTML = ICON;
  }

  applyLearningIcon();
  document.addEventListener('DOMContentLoaded', applyLearningIcon, { once: true });
  new MutationObserver(applyLearningIcon).observe(document.documentElement, { childList: true, subtree: true });
})();
